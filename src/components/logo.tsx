import { Printer } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useBrand } from '@/lib/brand-context';

export function Logo({
  className,
  size = 'default',
}: {
  className?: string;
  size?: 'sm' | 'default' | 'lg';
}) {
  const dims = {
    sm: { box: 'h-8 w-8', icon: 16, text: 'text-base' },
    default: { box: 'h-10 w-10', icon: 20, text: 'text-lg' },
    lg: { box: 'h-14 w-14', icon: 28, text: 'text-2xl' },
  }[size];

  const { logoUrl } = useBrand();

  return (
    <div className={cn('flex items-center gap-2.5', className)}>
      <div
        className={cn(
          'relative flex items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-primary to-sky-600 shadow-glow',
          dims.box
        )}
      >
        {logoUrl ? (
          <img
            src={logoUrl}
            alt="Online Print 4U logo"
            className="h-full w-full object-contain"
          />
        ) : (
          <Printer className="text-white" size={dims.icon} strokeWidth={2} />
        )}
      </div>
      <div className="flex flex-col leading-none">
        <span className={cn('font-display font-bold tracking-tight text-foreground', dims.text)}>
          Online Print <span className="text-primary">4U</span>
        </span>
        {size !== 'sm' && (
          <span className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground">
            Fast • Easy • Reliable
          </span>
        )}
      </div>
    </div>
  );
}
