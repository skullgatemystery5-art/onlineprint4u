-- Online Print 4U — Complete Schema
-- This app uses Firebase Authentication, NOT Supabase Auth.
-- All RLS policies use TO anon, authenticated so the anon-key client can operate.

-- profiles (text id for Firebase UIDs)
...
CREATE POLICY "order_files_public_read" ON storage.objects
  FOR SELECT TO anon, authenticated USING (bucket_id = 'order-files');

DROP POLICY IF EXISTS "order_files_public_insert" ON storage.objects;
CREATE POLICY "order_files_public_insert" ON storage.objects
  FOR INSERT TO anon, authenticated WITH CHECK (bucket_id = 'order-files');

DROP POLICY IF EXISTS "order_files_public_update" ON storage.objects;
CREATE POLICY "order_files_public_update" ON storage.objects
  FOR UPDATE TO anon, authenticated USING (bucket_id = 'order-files') WITH CHECK (bucket_id = 'order-files');

DROP POLICY IF EXISTS "order_files_public_delete" ON storage.objects;
CREATE POLICY "order_files_public_delete" ON storage.objects
  FOR DELETE TO anon, authenticated USING (bucket_id = 'order-files');
...
ON CONFLICT (courier_type) DO UPDATE SET
  label = EXCLUDED.label, base_rate = EXCLUDED.base_rate,
  per_kg_rate = EXCLUDED.per_kg_rate, estimated_days = EXCLUDED.estimated_days;
...
SET label = EXCLUDED.label, base_rate = EXCLUDED.base_rate,
    per_kg_rate = EXCLUDED.per_kg_rate, estimated_days = EXCLUDED.estimated_days,
...
ON CONFLICT (code) DO UPDATE SET
  ...
  value = EXCLUDED.value, min_order = EXCLUDED.min_order,
  max_discount = EXCLUDED.max_discount, active = EXCLUDED.active;
