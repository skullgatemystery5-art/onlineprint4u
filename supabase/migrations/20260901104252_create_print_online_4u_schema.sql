/*
# Online Print 4U — Complete Schema (Consolidated)
...
All RLS policies use `TO anon, authenticated` so the anon-key client can operate.

1. New Tables
- `profiles` — user profile rows (id is text for Firebase UIDs, no FK to auth.users)
- `addresses` — saved delivery addresses per user (user_id is text)
...
2. Security (RLS)
- All tables use `TO anon, authenticated` policies (Firebase auth, no Supabase session)
- Public-read tables: coupons (active), pricing_rates, shipping_rates, reviews (active), site_settings
- Contact messages: anyone may insert; anon+authenticated read/delete
- Orders/addresses/profiles: full CRUD for anon+authenticated

3. Functions / Triggers
- `is_admin()` SECURITY DEFINER helper checking profiles.role = 'admin' (casts auth.uid() to text)
...
5. Storage
- Creates public bucket `order-files` for customer-uploaded documents
- Public read; anon+authenticated upload/update/delete

6. Seed Data
- Default pricing rates for all paper types (70/75/85/100 GSM, B&W/Color, single/double)
- Binding rates (none, spiral, soft, hard, thesis)
- Add-on rates (premium photo, lamination)
- Shipping rates (local delivery for Patna)
- Default coupons (WELCOME50, PRINT10)
- Customer reviews
- Default site settings
*/

-- profiles (text id for Firebase UIDs, no FK to auth.users)
...
-- addresses (text user_id for Firebase UIDs, no FK to auth.users)
...
-- orders (text user_id for Firebase UIDs, no FK to auth.users)
...
-- Helper function to check admin role (cast auth.uid() to text for Firebase UIDs)
...
-- Profiles policies (anon+authenticated for Firebase auth)
...
DROP POLICY IF EXISTS "status_log_select_all" ON public.order_status_log;
CREATE POLICY "status_log_select_all" ON public.order_status_log FOR SELECT
  TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "status_log_insert_all" ON public.order_status_log;
CREATE POLICY "status_log_insert_all" ON public.order_status_log FOR INSERT
  TO anon, authenticated WITH CHECK (true);

-- Coupons policies (public read of active, admin CRUD)
...
CREATE POLICY "coupons_insert_all" ON public.coupons FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "coupons_update_all" ON public.coupons;
CREATE POLICY "coupons_update_all" ON public.coupons FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "coupons_delete_all" ON public.coupons;
CREATE POLICY "coupons_delete_all" ON public.coupons FOR DELETE
  TO anon, authenticated USING (true);

-- Pricing rates policies (public read, admin CRUD)
...
CREATE POLICY "pricing_select_public" ON public.pricing_rates FOR SELECT
  TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "pricing_insert_all" ON public.pricing_rates;
CREATE POLICY "pricing_insert_all" ON public.pricing_rates FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "pricing_update_all" ON public.pricing_rates;
CREATE POLICY "pricing_update_all" ON public.pricing_rates FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pricing_delete_all" ON public.pricing_rates;
CREATE POLICY "pricing_delete_all" ON public.pricing_rates FOR DELETE
  TO anon, authenticated USING (true);

-- Shipping rates policies (public read, admin CRUD)
...
CREATE POLICY "shipping_select_public" ON public.shipping_rates FOR SELECT
  TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "shipping_insert_all" ON public.shipping_rates;
CREATE POLICY "shipping_insert_all" ON public.shipping_rates FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "shipping_update_all" ON public.shipping_rates;
CREATE POLICY "shipping_update_all" ON public.shipping_rates FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "shipping_delete_all" ON public.shipping_rates;
CREATE POLICY "shipping_delete_all" ON public.shipping_rates FOR DELETE
  TO anon, authenticated USING (true);

-- Reviews policies (public read of active, admin CRUD)
...
CREATE POLICY "reviews_insert_all" ON public.reviews FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "reviews_update_all" ON public.reviews;
CREATE POLICY "reviews_update_all" ON public.reviews FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "reviews_delete_all" ON public.reviews;
CREATE POLICY "reviews_delete_all" ON public.reviews FOR DELETE
  TO anon, authenticated USING (true);

-- Contact messages policies (anyone insert, anon+authenticated read/delete)
...
CREATE POLICY "contact_insert_any" ON public.contact_messages FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "contact_select_all" ON public.contact_messages;
CREATE POLICY "contact_select_all" ON public.contact_messages FOR SELECT
  TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "contact_delete_all" ON public.contact_messages;
CREATE POLICY "contact_delete_all" ON public.contact_messages FOR DELETE
  TO anon, authenticated USING (true);

-- Site settings policies (public read, admin CRUD)
...
CREATE POLICY "site_settings_insert_all" ON public.site_settings FOR INSERT
  TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "site_settings_update_all" ON public.site_settings;
CREATE POLICY "site_settings_update_all" ON public.site_settings FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "order_files_anon_insert" ON storage.objects;
CREATE POLICY "order_files_anon_insert" ON storage.objects
  FOR INSERT
  TO anon, authenticated
  WITH CHECK (bucket_id = 'order-files');

DROP POLICY IF EXISTS "order_files_anon_update" ON storage.objects;
...
DROP POLICY IF EXISTS "order_files_anon_delete" ON storage.objects;
